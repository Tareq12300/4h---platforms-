...
STOCH_RSI_PERIOD = int(os.environ.get("STOCH_RSI_PERIOD", "14"))
...
MAX_STOCH_BUY = float(os.environ.get("MAX_STOCH_BUY", "20"))
...
# إعدادات المقارنة الهجينة — جميعها قابلة للتغيير من المتغيرات
USE_CLOSED_CANDLE = os.environ.get("USE_CLOSED_CANDLE", "false").lower() == "true"
COMPARE_PRICE_WITH_PREVIOUS_CANDLE = os.environ.get("COMPARE_PRICE_WITH_PREVIOUS_CANDLE", "true").lower() == "true"
COMPARE_STOCH_WITH_PREVIOUS_CANDLE = os.environ.get("COMPARE_STOCH_WITH_PREVIOUS_CANDLE", "true").lower() == "true"
COMPARE_MACD_WITH_PREVIOUS_CANDLE = os.environ.get("COMPARE_MACD_WITH_PREVIOUS_CANDLE", "true").lower() == "true"
REQUIRE_PRICE_RISING = os.environ.get("REQUIRE_PRICE_RISING", "true").lower() == "true"
REQUIRE_STOCH_CROSS = os.environ.get("REQUIRE_STOCH_CROSS", "true").lower() == "true"
REQUIRE_STOCH_RISING = os.environ.get("REQUIRE_STOCH_RISING", "true").lower() == "true"
REQUIRE_MACD_POSITIVE = os.environ.get("REQUIRE_MACD_POSITIVE", "true").lower() == "true"
REQUIRE_MACD_HISTOGRAM_RISING = os.environ.get("REQUIRE_MACD_HISTOGRAM_RISING", "true").lower() == "true"
REQUIRE_MACD_SWITCH_TO_POSITIVE = os.environ.get("REQUIRE_MACD_SWITCH_TO_POSITIVE", "false").lower() == "true"
REQUIRE_VOLUME_SPIKE = os.environ.get("REQUIRE_VOLUME_SPIKE", "true").lower() == "true"
VOLUME_COMPARISON_MODE = os.environ.get("VOLUME_COMPARISON_MODE", "average").strip().lower()
VOLUME_AVERAGE_CANDLES = max(1, int(os.environ.get("VOLUME_AVERAGE_CANDLES", "10")))
...
def detect_volume_spike(market_data: dict) -> dict:
    """يقارن فوليوم شمعة التحليل بمتوسط الشموع أو بالشمعة السابقة."""
    closes = market_data.get("closes", [])
    quote_volumes = market_data.get("quote_volumes", [])
    base_volumes = market_data.get("base_volumes", [])
    mode = VOLUME_COMPARISON_MODE if VOLUME_COMPARISON_MODE in {"average", "previous"} else "average"
    required = VOLUME_AVERAGE_CANDLES + 1 if mode == "average" else 2

    if len(closes) < required:
        return {"spike": False, "ratio": 0.0, "current_volume_usd": 0.0,
                "previous_volume_usd": 0.0, "baseline_volume_usd": 0.0,
                "comparison_mode": mode, "enough_data": False}

    if len(quote_volumes) >= required:
        usd_volumes = [safe_float(v) for v in quote_volumes]
    elif len(base_volumes) >= required:
        usd_volumes = [safe_float(v) * safe_float(c) for v, c in zip(base_volumes, closes)]
    else:
        return {"spike": False, "ratio": 0.0, "current_volume_usd": 0.0,
                "previous_volume_usd": 0.0, "baseline_volume_usd": 0.0,
                "comparison_mode": mode, "enough_data": False}

    current_usd = usd_volumes[-1]
    previous_usd = usd_volumes[-2]
    if mode == "previous":
        baseline_usd = previous_usd
    else:
        window = usd_volumes[-(VOLUME_AVERAGE_CANDLES + 1):-1]
        baseline_usd = sum(window) / len(window) if window else 0.0
    ratio = current_usd / baseline_usd if baseline_usd > 0 else 0.0
    return {"spike": ratio >= MIN_VOLUME_RATIO, "ratio": round(ratio, 2),
            "current_volume_usd": current_usd, "previous_volume_usd": previous_usd,
            "baseline_volume_usd": baseline_usd, "comparison_mode": mode, "enough_data": True}
...
def analyze_signal(
    ...
    analysis_close = safe_float(data.get("analysis_close", price))
    previous_close = safe_float(data.get("previous_close", 0))

    if (COMPARE_PRICE_WITH_PREVIOUS_CANDLE and REQUIRE_PRICE_RISING
            and previous_close > 0 and analysis_close <= previous_close):
        return None
    ...
    if (
        stoch_k >= MAX_STOCH_BUY
        or stoch_d >= MAX_STOCH_BUY
    ):
        return None
    ...
    if REQUIRE_VOLUME_RATIO or REQUIRE_VOLUME_SPIKE:
    ...
    stoch_crossed = stoch_k_prev <= stoch_d_prev and stoch_k > stoch_d
    stoch_turning_up = stoch_k > stoch_k_prev and stoch_d > stoch_d_prev
    stoch_gap = max(stoch_d - stoch_k, 0)

    if COMPARE_STOCH_WITH_PREVIOUS_CANDLE:
        if REQUIRE_STOCH_CROSS and not stoch_crossed:
            return None
        if REQUIRE_STOCH_RISING and not stoch_turning_up:
            return None
    elif REQUIRE_STOCH_CROSS and not stoch_above:
        return None
    ...
    if not stoch_above and not early_stoch:
        return None

    if stoch_crossed:
        score += 2
        reasons.append("Stoch RSI: تقاطع صاعد حقيقي بين الشمعتين 📈")
    elif stoch_above:
        score += 1
        reasons.append("Stoch RSI: K أعلى من D")
    else:
        score += 1
        reasons.append(
    ...
    if COMPARE_MACD_WITH_PREVIOUS_CANDLE:
        if REQUIRE_MACD_HISTOGRAM_RISING and not macd_improving:
            return None
    elif REQUIRE_MACD_RISING and not macd_improving:
        return None

    if REQUIRE_MACD_POSITIVE and histogram <= 0:
        return None
    if REQUIRE_MACD_SWITCH_TO_POSITIVE and not macd.get("switched_to_rising"):
        return None
    if histogram < 0 and not ALLOW_NEGATIVE_MACD:
        return None
    ...
    confirmed_ready = (
        (stoch_crossed if REQUIRE_STOCH_CROSS else stoch_above)
        and (histogram > 0 or not REQUIRE_CONFIRMED_MACD_POSITIVE)
    )
    ...
    if volume_spike:
        ...
        reasons.append(
            ...
            f"من خط الأساس `{volume_ratio:.2f}x` 🔥"
        )
    else:
        reasons.append(
        ...
        try:
            ...
            for index, coin in enumerate(
                ...
                analysis_market_data = (
                    ...
                    if USE_CLOSED_CANDLE else market_data
                )
                closes = analysis_market_data["closes"]
                ...
                coin["analysis_close"] = closes[-1]
                coin["previous_close"] = closes[-2]
